...
function getRandomArrayLength(array) {
  return cloneArray = array.slice(0, getRandomNumber(1, array.length))
}
...
function generateNotices() {
  let notices = []
  let noticeAvatars = shuffleArray(generateAvatars())
  let noticeTitles = shuffleArray(DATA_NOTICES.TITLES)
  for (let i = 0; i < DATA_NOTICES.COUNT_USERS; i++) {
    let locationX = getRandomNumber(DATA_NOTICES.MIN_X, DATA_NOTICES.MAX_X)
    let locationY = getRandomNumber(DATA_NOTICES.MIN_Y, DATA_NOTICES.MAX_Y)
    notices.push({
      ...
      offer:
      {
        ...
        features: getRandomArrayLength(DATA_NOTICES.FEATURES),
        description: '',
